...
async function createServer(channel, { cfg, wc, baseUrl }, addInitError) {
  const bablePlugins = [pluginTransformReactJsx]
  const tempConfig = cfg.useVue
    ? vue({})
    : reactPlugin({
        jsxRuntime: 'classic',
        babel: {
  ...
  const moduleGraph = new ModuleGraph((url, ssr) =>
    pluginContainer.resolveId(url, undefined, {
      ssr,
    }),
  )
  ...
  try {
    ...
    ;(initError = error),
      (initError.message = error.message.replace(
        /^Build failed with \d+ error.*:/,
        'Failed to start compilation server:',
      ))
    const { location } =
      (error.errors == null ? undefined : error.errors[0]) || {}

    if (location?.file) {
      initError.id = `${location.file}:${location?.line || 0}:${
        location?.column || 0
      }`
      initError.loc = location
      initError.frame = generateCodeFrame(
        wc.tree[Nlt(wc.ref.id, location.file).slice(1)],
        location,
      )
    }
  }
