    ...
    viteWorker.onmessage = (event) => {
      ...
      switch (data.cmd) {
        ...
        case 'compile-context-inited': {
          ...
          iframe.setAttribute(
            'style',
            'width: 100%; height: 100vh; border: 0; outline: 0;',
          )
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', function () {
    const wb = new Workbox(
      `${isProd ? '/vitesandbox-client' : ''}/serviceWorker.js`,
      {
        updateViaCache: 'none',
        type: 'module',
      },
